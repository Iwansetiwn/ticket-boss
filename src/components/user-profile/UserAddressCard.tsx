...
const getInitialForm = (user: AuthUser): AddressForm => ({
...
export default function UserAddressCard({ user }: Props) {
  ...
  const [form, setForm] = useState<AddressForm>(() => getInitialForm(user));
  ...
  const handleChange =
    (key: keyof AddressForm) => (event: React.ChangeEvent<HTMLInputElement>) => {
          ...
          <div>
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-7 2xl:gap-x-32">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-gray-800 dark:text-white/90">
                  {user.cityState || "Add your city & state"}
                </p>
              </div>
              ...
              <div>
                ...
                <p className="text-sm font-medium text-gray-800 dark:text-white/90">
                  {user.postalCode || "Add postal code"}
                </p>
              </div>

              <div>
                <p className="mb-2 text-xs leading-normal text-gray-500 dark:text-gray-400">TAX ID</p>
                <p className="text-sm font-medium text-gray-800 dark:text-white/90">{user.taxId || "Add tax id"}</p>
              </div>
            </div>
          </div>
        ...
        <div className="relative w-full p-4 overflow-y-auto bg-white no-scrollbar rounded-3xl dark:bg-gray-900 lg:p-11">
          <div className="px-2 pr-14">
            ...
            <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
              Update where you and your business operate from.
            </p>
          </div>
          <form className="flex flex-col gap-6" onSubmit={handleSave}>
            <div className="custom-scrollbar max-h-[350px] space-y-5 overflow-y-auto px-2">
              <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                ...
                <div>
                  ...
                  <Input type="text" value={form.cityState} onChange={handleChange("cityState")} />
                </div>

                <div>
                  <Label>Postal Code</Label>
                  <Input type="text" value={form.postalCode} onChange={handleChange("postalCode")} />
                </div>

                <div>
                  <Label>TAX ID</Label>
                  <Input type="text" value={form.taxId} onChange={handleChange("taxId")} />
                </div>
              </div>
            </div>
