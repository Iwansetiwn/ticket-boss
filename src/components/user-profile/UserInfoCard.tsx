...
const getInitialForm = (user: AuthUser): InfoForm => ({
...
export default function UserInfoCard({ user }: Props) {
  ...
  const [form, setForm] = useState<InfoForm>(() => getInitialForm(user));
  ...
  const displayLastName = user.lastName || "Add last name";
  const displayBio = user.bio || "Add a short bio";
  const displayPhone = user.phone || "Add your phone";

  const handleChange =
    (key: keyof InfoForm) => (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        <div>
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-7 2xl:gap-x-32">
            ...
            <div>
              ...
              <p className="text-sm font-medium text-gray-800 dark:text-white/90">{displayLastName}</p>
            </div>
            ...
            <div>
              ...
              <p className="text-sm font-medium text-gray-800 dark:text-white/90">{displayPhone}</p>
            </div>

            <div className="lg:col-span-2">
              <p className="mb-2 text-xs leading-normal text-gray-500 dark:text-gray-400">Bio</p>
              <p className="text-sm font-medium text-gray-800 dark:text-white/90">{displayBio}</p>
            </div>
          </div>
        </div>
      ...
      <Modal isOpen={isOpen} onClose={closeModal} className="max-w-[700px] m-4">
        <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
          ...
          <form className="flex flex-col gap-6" onSubmit={handleSave}>
            <div className="custom-scrollbar max-h-[450px] space-y-5 overflow-y-auto px-2">
              <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                ...
                <div className="lg:col-span-2">
                  <Label>Bio</Label>
                  <Input type="text" value={form.bio} onChange={handleChange("bio")} />
                </div>
              </div>
            </div>
